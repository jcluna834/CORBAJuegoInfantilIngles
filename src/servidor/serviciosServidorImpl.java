...
public class serviciosServidorImpl extends serviciosServidorPOA {

    List<Archivo> diccionarios;
    List<estudianteCllbck> estudiantes;
    estudianteCllbck pendiente;

    public serviciosServidorImpl() {
        super();
        diccionarios = new ArrayList<>();
        estudiantes = new ArrayList<>();
    }

    //edita una palabra del diccionario con el codigo ingresado, las palabras deben tener el formato:
    // codigo;palabraEnIngles;palabraEnEspañol
    // se modificará la palabra que coincida con la misma estructura y los mismos valores
    public boolean modificarPalabra(String palabraAnterior, String palabraNueva, int codigo) {
        ...
        if (diccionario != null) {
            return cambiarPalabra(diccionario, palabraAnterior, palabraNueva);
        }
        return false;
    }
    ...
    public boolean borrarDiccionario(int codigo) {
        if (!diccionarios.isEmpty()) {
            return eliminarDiccionario(codigo);
        } else {
            return false;
        }
    }
    ...
    public void iniciarCompletaLetra(sop_corba.estudianteCllbck estudiante) {
        if (pendiente == null) {
            ...
            pendiente.chat(estudiante);
            estudiante.chat(pendiente);
            pendiente = null;
        }
    }
    ...
    // Elimina el diccionario de la lista de diccionarios
    private boolean eliminarDiccionario(int codigo) {
        if (!diccionarios.isEmpty()) {
            for (Archivo d : diccionarios) {
                if (d.getCodigo() == codigo) {
                    return diccionarios.remove(d);
                }
            }
        }
        return false;
    ...
    private List<String> cargarPalabras() {
        ...
        diccionarios.stream().forEach((diccionario) -> {
            try {
